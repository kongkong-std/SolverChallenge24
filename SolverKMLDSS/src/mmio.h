...
int mm_write_mtx_crd(char fname[], int M, int N, int nz, int Irow[], int Jcol[], double val[], MM_typecode matcode);
int mm_read_mtx_crd_data(FILE *f, int M, int N, int nz, int Irow[], int Jcol[], double val[], MM_typecode matcode);
int mm_read_mtx_crd_entry(FILE *f, int *Irow, int *Jcol, double *real, double *img, MM_typecode matcode);

int mm_read_unsymmetric_sparse(const char *fname, int *M_, int *N_, int *nz_, double **val_, int **Irow_, int **Jcol_);
...
int mm_read_mtx_crd(char *fname, int *M, int *N, int *nz, int **Irow, int **Jcol, double **val, MM_typecode *matcode)
{
    int ret_code;
    FILE *f;
    ...
    *Irow = (int *)malloc(*nz * sizeof(int));
    *Jcol = (int *)malloc(*nz * sizeof(int));
    *val = NULL;

    if (mm_is_complex(*matcode))
    {
        *val = (double *)malloc(*nz * 2 * sizeof(double));
        ret_code = mm_read_mtx_crd_data(f, *M, *N, *nz, *Irow, *Jcol, *val, *matcode);
        if (ret_code != 0)
            return ret_code;
    }
...
int mm_write_mtx_crd(char fname[], int M, int N, int nz, int Irow[], int Jcol[], double val[], MM_typecode matcode)
{
    ...
    if (mm_is_pattern(matcode))
        ...
        for (i = 0; i < nz; i++)
            fprintf(f, "%d %d %20.16g %20.16g\n", Irow[i], Jcol[i], val[2 * i], val[2 * i + 1]);
    else
    {
        if (f != stdout)
            fclose(f);
...
int mm_read_mtx_crd_data(FILE *f, int M, int N, int nz, int Irow[], int Jcol[], double val[], MM_typecode matcode)
{
    ...
    {
        for (i = 0; i < nz; i++)
            if (fscanf(f, "%d %d %lg %lg\n", &Irow[i], &Jcol[i], &val[2 * i], &val[2 * i + 1]) != 4)
                return MM_PREMATURE_EOF;
    }
    else if (mm_is_real(matcode))
    {
        ...
        for (i = 0; i < nz; i++)
        {
            ...
            if (fscanf(f, "%d %d %lg", &Irow[i], &Jcol[i], &val[i]) != 3)
                return MM_PREMATURE_EOF;
        }
    }
    ...
    else if (mm_is_pattern(matcode))
    {
        ...
        for (i = 0; i < nz; i++)
            if (fscanf(f, "%d %d", &Irow[i], &Jcol[i]) != 2)
                return MM_PREMATURE_EOF;
    }
    else
    {
...
int mm_read_mtx_crd_entry(FILE *f, int *Irow, int *Jcol, double *real, double *imag, MM_typecode matcode)
{
    if (mm_is_complex(matcode))
    {
        if (fscanf(f, "%d %d %lg %lg", Irow, Jcol, real, imag) != 4)
            return MM_PREMATURE_EOF;
    }
    else if (mm_is_real(matcode))
    {
...
int mm_read_unsymmetric_sparse(const char *fname, int *M_, int *N_, int *nz_, double **val_, int **Irow_, int **Jcol_)
{
    ...
    *Jcol_ = Jcol;
    ...
    {
        fscanf(f, "%d %d %lg\n", &Irow[i], &Jcol[i], &val[i]);
        Irow[i]--; /* adjust from 1-based to 0-based */
        Jcol[i]--;
    }
    fclose(f);
